...
from pureml.supervised.tree_based.xgboost_tree_regressor import XGBoostTreeRegressor
...
class XGBoostRegressor:
    ...
    def fit(
        ...
        tree_max_features = self._resolve_max_features(X.shape[1])
        ...
        for estimator_index in range(self.n_estimators):
            grad = self._gradients(y, current_prediction)
            hess = self._hessians(y, current_prediction)
            ...
            tree.fit(X[sample_indices], grad[sample_indices], hess[sample_indices])
    ...
    def _gradients(self, y, prediction):
        return prediction - y

    def _hessians(self, y, prediction):
